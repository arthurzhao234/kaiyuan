 ...
 * */
void
print_stackframe(void) {//功能：打印栈针

     /* LAB1 YOUR CODE : STEP 1 */
     /* (1) call read_ebp() to get the value of ebp. the type is (uint32_t); 栈底寄存器
      * (2) call read_eip() to get the value of eip. the type is (uint32_t);指令指针寄存器
    ...
    uint32_t ebp=read_ebp(), eip=read_eip(); //uint32_t只是int的别名,通过汇编来获取了寄存器的值
    int i;
    for(i=0;i<STACKFRAME_DEPTH;i++){
        cprintf("ebp:0x%08x, eip:0x%08x args", ebp ,eip);//%08是格式化的后缀
        uint32_t *args=(uint32_t *)ebp+2; //将argument的寄存器+2
        int j;
        for(j=0;j<4;j++){
            cprintf("%08x",args[i]);//3.2打印出值
        }
        cprintf("\n");//3.3所写
        print_debuginfo(eip-1);//打印工作,3.4直接调用的
        eip=((uint32_t *)eip) [1];//3.5迭代eip、ebp两个数
        ebp=((uint32_t *)ebp) [0];
